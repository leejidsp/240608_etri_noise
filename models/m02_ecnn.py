...
class M02ECNN(nn.Module):
    """
    """
    def __init__(self, h):
        super(M02ECNN, self).__init__()
        ...
        # 3-1. Refiner
        self.env_refiner = AutoencoderCNN(h.EnvironmentalRefiner)
        
        # 3-2. Quantizer 
        h_env_q = h.EnvironmentalQuantizer
        max_n_q, nbins = self._calculate_q_params(h_env_q)
        print("ENV: max_n_q:{}, nbins:{} => {} kbps".format(max_n_q, nbins, h_env_q.target_bandwidths[0]))
       
        self.env_fr = h_env_q.frame_rate
        self.env_bw = h_env_q.target_bandwidths[-1]

        self.env_quantizer = ResidualVectorQuantizer(
                        dimension=h_env_q.dimension,
                        n_q=max_n_q,
                        bins=nbins,
                        decay=h_env_q.decay,
                        )
    ...
    def forward(self, x, quantize=[1, 1, 1], centroid_dict=None, inference=False, **kwargs):
        assert quantize in [[1, 1, 1], [1, 1, 0], [1, 0, 0]]
        ...
        env_raw = spk_raw - spk_dec


        ### 3. Encode environment
        env_enc = self.env_refiner.encode(env_raw)
        if not inference:
            env_q_result = self.env_quantizer.forward(env_enc, sample_rate=self.env_fr, bandwidth=self.env_bw)
            q_env_enc = env_q_result.quantized
        else:
            env_codes = self.env_quantizer.encode(env_enc, sample_rate=self.env_fr, bandwidth=self.env_bw)
            q_env_enc = self.env_quantizer.decode(env_codes)
        env_dec = self.env_refiner.decode(q_env_enc)


        ### 4. Generate waveform 
        if quantize == [1, 1, 1]:
            dec_feature = lin_dec + spk_dec + env_dec
        elif quantize == [1, 1, 0]:
    ...
    def inference_tag(self, x, q_tag, centroid_dict):

        if q_tag == 'W_LSE':
            out, _ = self.forward(x, quantize=[1, 1, 1], centroid_dict=centroid_dict, inference=True)
        if q_tag == 'W_LS':
            out, _ = self.forward(x, quantize=[1, 1, 0], centroid_dict=centroid_dict, inference=True)
        if q_tag == 'W_L':
            out, _ = self.forward(x, quantize=[1, 0, 0], centroid_dict=centroid_dict, inference=True)
        if q_tag == 'W_S':
            out1, _ = self.forward(x, quantize=[1, 1, 0], centroid_dict=centroid_dict, inference=True)
            out2, _ = self.forward(x, quantize=[1, 0, 0], centroid_dict=centroid_dict, inference=True)
            out = out1 - out2
        if q_tag == 'W_E':
            out1, _ = self.forward(x, quantize=[1, 1, 1], centroid_dict=centroid_dict, inference=True)
            out2, _ = self.forward(x, quantize=[1, 1, 0], centroid_dict=centroid_dict, inference=True)
            out = out1 - out2
...
class AutoencoderCNN(nn.Module):
    ...
    def __init__(self, h):
        ...
        for i in range(h.num_layers):
            in_ch = h.in_channels if i==0 else h.hidden_dimension
            out_ch = h.embed_channels if (i+1)==h.num_layers else h.hidden_dimension
            encoder_list.append(nn.Conv1d(in_ch, out_ch, kernel_size=h.kernel_size, padding=(h.kernel_size-1)//2))
            if (i+1)!=h.num_layers:
                encoder_list.append(nn.LeakyReLU())
        self.encoder = nn.Sequential(*encoder_list)
        ...
        for i in range(h.num_layers):
            in_ch = h.embed_channels if i==0 else h.hidden_dimension
            out_ch = h.in_channels if (i+1)==h.num_layers else h.hidden_dimension
            decoder_list.append(nn.Conv1d(in_ch, out_ch, kernel_size=h.kernel_size, padding=(h.kernel_size-1)//2))
            if (i+1)!=h.num_layers:
                decoder_list.append(nn.LeakyReLU())
        self.decoder = nn.Sequential(*decoder_list)
