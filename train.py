...
def main(cfg):
    ...
    noise_path_loader = DataLoader(noise_path_set, batch_size=1, shuffle=True)
    train_path_loader = DataLoader(train_path_set, batch_size=cfg.training.path_batch_size, shuffle=True)

    # Validation
    valid_noise_rng = np.random.default_rng(seed=cfg.record.eval_seed+100)
    valid_noise_indices = valid_noise_rng.integers(0, len(noise_path_set), size=cfg.record.eval_num)
    valid_set = load_dataset('valid', cfg.data, num_file=cfg.record.eval_num,
                                noise_path_set=noise_path_set, 
                                noise_indices=valid_noise_indices,
    ...
    # Tensorboard
    tb_noise_rng = np.random.default_rng(seed=cfg.record.tb_seed+100)
    tb_noise_indices = tb_noise_rng.integers(0, len(noise_path_set), size=cfg.record.tb_num)
    #tb_noise_indices = np.random.randint(0, len(noise_path_set), size=cfg.record.tb_num)
    tb_set = load_dataset('valid', cfg.data, num_file=cfg.record.tb_num,
                                noise_path_set=noise_path_set, 
                                noise_indices=tb_noise_indices,
                                frame_runner=frame_runner,
                                seed=cfg.record.tb_seed)
    tb_loader_dict = {}
    for snr in cfg.data.mix.snr_list:
        key = str(snr)+'dB' if snr != 'clean' else 'clean'
    ...
    data_dict = {
            'noise_path_loader': noise_path_loader,
