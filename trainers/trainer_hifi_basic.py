...
class TrainerHiFiBasic(object):
    def __init__(self, data_dict, model_dict, opt_dict, loss_function, cfg):
        ...
        # Data
        self.noise_path_loader  = data_dict['noise_path_loader']
        ...
        self.q_modes            = cfg.training.q_modes
        self.targets            = cfg.training.targets
    ...
    def _train_one_epoch(self, epoch):
        ...
        for path_list in pbar:
            ...
            train_frame_set = load_dataset('train_frame', self.data, 
                                            path_list=path_list, 
                                            noise_loader=self.noise_path_loader,
            ...
            # ===== Main Part =====
            # For each filebatch
            for i, (x, c_x, _, _) in enumerate(train_frame_loader):
                # x :                     torch.FloatTensor   : input frames    # (B, T)
                real_x = x.unsqueeze(1).cuda()   # (B, 1, T)
                real_c_x = c_x.unsqueeze(1).cuda()
                
                qidx = i % len(self.q_modes)
                if self.targets[qidx] == 'mix':
                    real_x_loss = real_x.detach()   # for loss calculation
                elif self.targets[qidx] == 'clean':
                    real_x_loss = real_c_x.detach()

                """ (1) Inference through autoencoder """
                gen_x, feat_dict = self.coder(real_x, self.q_modes[qidx], self.centroid_dict)  # (B, 1, T)
